...
export const TreeView: React.FC<TreeViewProps> = ({ treeData }) => {
  const tree = useRef<TreeRef>(null)
  const [viewState, setViewState] = useState<TreeViewState>(viewStateInitial)
  const [search, setSearch] = useState<string | undefined>('')

  const dataProvider = useMemo(
    () => new CustomTreeDataProvider<string>(treeData),
    [treeData],
  )
  ...
  return (
    <div className="mx-auto flex h-screen w-[400px] flex-col gap-1 pt-[100px]">
      {/* <form
        onSubmit={onSubmit}
