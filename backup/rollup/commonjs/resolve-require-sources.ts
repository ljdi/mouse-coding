...
export interface RequireResolver {
  getWrappedIds: () => string[];
  isRequiredId: (id: string) => boolean;
  shouldTransformCachedModule: ShouldTransformCachedModuleHook;
  resolveRequireSourcesAndUpdateMeta: (
    rollupContext: TransformPluginContext,
  ) => (
    parentId: string,
    isParentCommonJS: boolean,
    parentMeta: any,
    sources: any,
  ) => Promise<unknown>;
  isCurrentlyResolving(source: any, parentId: any): any;
}

export function getRequireResolver(
  extensions: string[],
  detectCyclesAndConditional: boolean,
  currentlyResolving: Map<string, unknown>,
): RequireResolver {
  const knownCjsModuleTypes: Record<
    string,
    boolean | typeof IS_WRAPPED_COMMONJS
  > = Object.create(null);
  const requiredIds: Record<string, boolean> = Object.create(null);
  const unconditionallyRequiredIds: Record<string, boolean> =
    Object.create(null);
  const dependencies: Record<string, Set<string>> = Object.create(null);
  const getDependencies = (id: string) =>
    dependencies[id] || (dependencies[id] = new Set<string>());

  const isCyclic = (id: string) => {
  ...
  const getTypeForFullyAnalyzedModule = (id: string) => {
  ...
  const setInitialParentType = (id: string, initialCommonJSType: boolean) => {
  ...
  return {
    ...
    isRequiredId: (id: string) => requiredIds[id],
        ...
        return requireTargets.map(({ id: dependencyId, allowProxy }, index) => {
          ...
          return {
            wrappedModuleSideEffects:
              isWrappedCommonJS &&
              rollupContext.getModuleInfo(dependencyId).moduleSideEffects,
            source: sources[index].source,
            id: allowProxy
              ? wrapId(
                  dependencyId,
                  isWrappedCommonJS ? WRAPPED_SUFFIX : PROXY_SUFFIX,
                )
              : dependencyId,
            isCommonJS,
          };
